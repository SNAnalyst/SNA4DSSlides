...
---
name: Homeplay_data_exploration
description: Data analysis before running any model
...
# - birthyear: numeric
class(network::get.vertex.attribute(Hookups, "birthyear"))
summary(network::get.vertex.attribute(Hookups, "birthyear"))

# - position (job): cat
class(network::get.vertex.attribute(Hookups, "position"))
table(network::get.vertex.attribute(Hookups, "position"))

# - race: cat
class(network::get.vertex.attribute(Hookups, "race"))
table(network::get.vertex.attribute(Hookups, "race"))

# - season (tv show): int but cat
class(network::get.vertex.attribute(Hookups, "season"))
table(network::get.vertex.attribute(Hookups, "season"))

# - sex: cat
class(network::get.vertex.attribute(Hookups, "sex"))
table(network::get.vertex.attribute(Hookups, "sex"))

# - sign (astrology)

class(network::get.vertex.attribute(Hookups, "sign"))
table(network::get.vertex.attribute(Hookups, "sign"))
```
]

---
# Q1 More data exploration

It would be a good idea to use a mixingmatrix to observe the categorical variables. 

For example here the .red[heterophilic] pattern is crystal clear.

.scroll-box-18[
```{r labERGM1_05, echo=TRUE}

snafun::make_mixingmatrix(Hookups, "sex")
```
]
...
.scroll-box-18[
```{r labERGM1_06, echo=TRUE}

snafun::plot_centralities(Hookups)
...
```{r labERGM1_07, echo=TRUE}

sna::triad.census(Hookups)

```

---
name: hypotheses formulation
description: Formulating hypotheses before running any model
# 2) What are the hypotheses formulated <br> in the book  for this example?
...
description: Running Baseline models
# 3) Nesting models - baseline models


```{r ergmImp,echo=FALSE}
library(ergm)
```


.scroll-box-18[
```{r labERGM1_08, echo=TRUE}

summary(Hookups ~ edges)
m0 <- ergm::ergm(Hookups ~ edges)
summary(m0)


m1 <- ergm::ergm(Hookups ~ edges + nodecov("birthyear"))
summary(m1)

m2 <- ergm::ergm(Hookups ~ edges + absdiff("birthyear"))
summary(m2)
...
.scroll-box-18[
```{r, abERGM1_09, echo=TRUE}

# nodecov("birthyear") performs less well than the other terms
...
---
name: testing_hypotheses
description: testing the hypotheses
# Testing the Hypotheses

Model specification

.scroll-box-18[
```{r, abERGM1_010, echo=TRUE}

# final model without + nodecov("birthyear")
m7 <- ergm::ergm(Hookups ~ edges + absdiff("birthyear") + nodematch("position") + 
                   nodematch("race") + nodematch("sex") + nodecov("season"))

summary(m7)

# Final Model in the book
m8 <- ergm::ergm(Hookups ~ edges + absdiff("birthyear") + nodecov("birthyear") +
                   nodematch("position") + nodematch("race") +
                    nodematch("sex") + nodecov("season") )
...
---
name: exploring_terms
description: Exploring other specifications
# Nodemix(sex)

.scroll-box-18[
```{r abERGM1_012, echo=TRUE}
m9 <- ergm::ergm(Hookups ~ edges + absdiff("birthyear") +     nodecov("birthyear") +
                   nodematch("position") + nodematch("race") +
                    nodemix("sex") + nodecov("season") )
summary(m9)
```
]
...
.scroll-box-18[
```{r abERGM1_013, echo=TRUE}
m10 <- ergm::ergm(Hookups ~ edges + absdiff("birthyear") +     nodecov("birthyear") +
                   nodematch("position") + nodematch("race") +
                    nodematch("sex", diff = TRUE) + nodecov("season") )
summary(m10)
```
]

---
# Comparing nodemix and nodematch diff = TRUE

.scroll-box-18[
```{r r abERGM1_014, echo=TRUE}

texreg::screenreg(list(m9, m10))
...
---
name: results
description: commenting on results
...
.scroll-box-18[
```{r, abERGM1_015, echo=TRUE}

texreg::screenreg(list(m7, m8))
...
<br>
<br>
## They are all dyadic independent 


---
# 3c) Is each of these variables exogenous <br> or endogenous?
...
<br>
<br>
## These covariates are all exogenous. 

### That's why all the models are dydic independent.


---
## 3d) Comment on the results. What is the substantive meaning of each <br> of these terms? Are the  hypotheses supported? Comment on the odd ratios too. 

.scroll-box-18[
```{r, abERGM1_016, echo=TRUE}

texreg::screenreg(list(m8))
...
### P hooking up: forming ties
 <br>
### the probability of hooking up decreases with difference in age
### the probability of hooking up does not increase nor decrease when people get older 
### the probability of hooking up increases when working in the same position
### the probability of hooking up increases between people that share the same race
### the probability of hooking up increases when people have different sex (heterophily)
### the probability of hooking up decreases for people that have entered the show in later seasons

---
# 3e) What is the difference between <br> absdiff and nodecov estimated on birthyear?

---
# 3e) What is the difference between <br> absdiff and nodecov estimated on birthyear?
<br>

## nodecov <- predicting ties as var increases. Older more hookups. More wealth more weddings
## absdiff <- predicting ties according to similarity in age/wealth.

---
name: adding_structural_terms
description: Adding structural undirected terms
# 4)  Formulate one additional hypothesis on the network’s structure and then nest another model,

### In the book there were 2:

- 1 There is a significant number of monogamous relationships
- 2 There is a significant number of characters in a relationship with two characters (different time) that are not in a relationship.
...
.red[degree1] - so we can observe the difference from monogamous relationships (degree 1)

- There is a significant number of characters in a relationship with two characters (different time) that are not in a relationship.

.red[dyadic shared partners] triangles are not going to close since it would imply same sex relationships.
    
---
# Fitting the model

.scroll-box-18[
```{r abERGM1_017, echo=TRUE}


m11 <- ergm::ergm(Hookups ~ edges + absdiff("birthyear") + nodecov("birthyear") +
                   nodematch("position") + nodematch("race") +
                    nodematch("sex") + nodecov("season") + degree(1) + dsp(1),
...
---
name: re_fitting_curved
description: Improving the model with curved term
# Re-Fitting the model with curved term

.scroll-box-18[
```{r abERGM1_018, echo=TRUE}
m12 <- ergm::ergm(Hookups ~ edges + absdiff("birthyear") + nodecov("birthyear") +
                    nodematch("position") + nodematch("race") +
                    nodematch("sex") + nodecov("season") + degree(1) + gwdsp(1, fixed = FALSE),
...
---
## Great uh? what else we might ever need?
<br>
<br>
<br>
<br>
