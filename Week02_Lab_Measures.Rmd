...
`snafun::g_transitivity()` is broken for specific graphs. This will be fixed this week.
<br><br>
--
In the tutorial, you are referred to the `snafun` website. This website is now superseded. 
For a full overview of the package and its functions use:

```{r updates, eval=FALSE, echo = TRUE}
snafun::browse_vignettes()  # a list of available vignettes will open
snafun::browse_cheatsheet() # the full cheatsheet will open in your default browser
```
...
This is also called "clustering coefficient".
...
In *directed* networks, it is common to consider directed ties _as being undirected_ when 
...
A measure of the fit of a partitioning. Partition the set of vertices into 
_c_ communities. 
Compares within-community edges with between-community edges for an unweighted undirected graph:

$$Q = \sum_{i = 1}^{c} (e_{i} - a_{i}^2)$$

where $e_{i}$ is the fraction of edges in the graph that stay within community _i_ 
and $a_{i}^2$ is the fraction of ends of edges that are attached to vertices in community _i_ (irrespective of where the edge goes). 
For a directed graph:

$$Q = \sum_{i = 1}^{c} (e_i - a_{i}^{in} * a_{i}^{out})$$


- Modularity is between ${\displaystyle [-1/2,1]}$. It is positive if the number of edges within communities exceeds the number 
expected in a graph with the same number of vertices and where each vertex keeps 
its degree (but edges are randomly attached). Value 0 represents perfect randomness of the wiring.

- In practice: modularity > 0.3 indicates a good community structure.

.font70[
Note: Modularity is between ${\displaystyle [-1,1]}$ for graphs with negative weights.
]
...
## .center[.White-inline[code:] .Gold-inline[45911523]]
