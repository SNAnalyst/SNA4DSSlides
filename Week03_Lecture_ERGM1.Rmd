...
yes, they are in the second exam
...
### .center[Introduction to the Experimental Method]
...
### .center[Deductive approaches: From Theory and Hypotheses to Empirical Testing]
...
.red[ERGMs are a class of network models for causal inference: In a deductive research approach, ERGMs can be used to translate theoretically derived hypotheses about relational mechanisms into statistical models and test whether the observed network structure is consistent with those hypotheses.]
...
### .red[Predictors] <- what we believe could explain it

The hypothesis is 
- A theoretically informed, testable answer to your research question that you subject to empirical evaluation.
- A proposed explanation of the underlying mechanism: how and why you expect a predictor to influence or generate the observed outcome.

We are explaining network structure, right? .red[Our outcome variable MUST be a network]
...
### In non-network regression models (OLS, GLM, …), we ask: Does this predictor help explain the outcome? (tehn interpret coefficient,sign, p-value)

### .red[We start with a theory → formulate a hypothesis → test it with data.]

### The basic question is: .blue[Did we observe a real relationship, or could we have seen this pattern just by chance?]

### The p-value is NOT the probability that our hypothesis is true.

### It tells us how surprising our data would be if there were actually no relationship.

### Think of it like this:

- Our result could easily happen by chance → not much evidence that it is a real effect.

- Our result would be exceptional by chance → stronger evidence that it is not random.

---
# Statistical Mindset - Moving on
<br>
### The mindset is the same with Networks, but the technicalities are different.
<br>
...
<br>
...explaining a network takes more work than explaining a non-relational observed variable!
...
<br>
### each student at JADS = 1 node
<br>
## Let's say that JADS organizes an Hackathon with 5K euros at stake 

## You have to form groups to work together every day: If you study together there is an edge between each pair of you

### I map down the study-together dynamics two days before the Hackathon


.red[Now ask: Would this network look like a random network?]
...
Connections will be different than random, they are driven by a purpose (e.g., everyone wants to connect with the best student to win.)
...
# Let's move to a more complex configuration (P1 model)
...
Dyad Census (4 patternsin directed networks)

Triad Census (16 patterns in directed networks)
...
- $P(N,\theta)$ - Probability of observing this real Network, with these parameters
