...
<iframe width="1212" height="682" src="assets/html/homeplay02_code.html" title="Homeplay 2 code" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" style="width:100%; height:100%;" allowfullscreen></iframe>
...
<!-- ```{r lab02__02bb, echo = FALSE, eval= FALSE, fig.height = 4, message=FALSE, warning=FALSE} -->
<!-- opar <- par() -->
<!-- par("mar" = c(2, 2, 2, 2)) -->
<!-- d <- density(rnorm(20)) -->
<!-- plot(d) -->
...
multiples
igraph::ends(blogosphere, multiples)
```
]
]
...
So, make sure to always pass an object of the correct class!
...
- Use our very helpful and extensive cheatsheet (see Canvas or the
[html5 slides](https://snanalyst.github.io/SNA4DSSlides/index.html))
...
<div class="extension-tile slide"><div>Exercise</div></div>
<div class="extension-tile editable"><div>How to know?</div></div>
<div class="extension-tile animate animated slideInLeft"><div>Exercise</div></div>
</div>

---
class: course-logo
name: exercise2
description: Do this exercise (2)

- load the `emon` dataset from the `network` package

- extract the `wichita` network from it

- calculate the global efficiency of the graph (use the default settings)

- calculate the local efficiency for each of the vertices (use the default settings)
...
Search the help of the packages and convert appropriately.

]

.right-column.scroll-box-30[
```{r wichita, echo = TRUE}
data(emon, package = "network")
wichita <- emon$Wichita
sna::efficiency(wichita)
wichita_i <- snafun::to_igraph(wichita)
igraph::global_efficiency(wichita_i)
igraph::local_efficiency(wichita_i)
```
]

???

There is no function for local efficiency in sna or network, it is in igraph
Convert the graph to igraph and then apply it
